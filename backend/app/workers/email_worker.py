...
def _identity() -> str:
    ...
    _worker_identity = f"{socket.gethostname()}:{os.getpid()}:{uuid4().hex[:8]}"
    return _worker_identity
...
def _release_lock(master_db: Session, state: EmailSyncState, *, owner: str | None = None, success: bool, error: str | None = None) -> None:
    now = datetime.now(timezone.utc)
    values = {
        "lock_owner": None,
        "lock_until": None,
        ...
        "last_error_message": None if success else error,
    }
    conditions = [EmailSyncState.id == state.id]
    if owner:
        conditions.append(EmailSyncState.lock_owner == owner)
    master_db.execute(update(EmailSyncState).where(*conditions).values(**values))
    if success:
        state.last_success_at = now
    master_db.commit()


def _run_due_tenant(master_db: Session, tenant: MasterTenantDatabase, state: EmailSyncState, *, owner: str) -> None:
    try:
        reconcile_tenant_email(master_db, tenant, owner=owner)
        _release_lock(master_db, state, owner=owner, success=True)
    except Exception as exc:  # noqa: BLE001
        logger.exception("Error sincronizando tenant %s: %s", tenant.company_id, exc)
        _release_lock(master_db, state, owner=owner, success=False, error=str(exc))
        ...
        try:
            ...
            for state in due_states:
                ...
                if not tenant or not tenant.database_url:
                    continue
                if not _acquire_lock(master_db, state, owner=owner):
                    continue
                _run_due_tenant(master_db, tenant, state, owner=owner)
        except Exception as exc:  # noqa: BLE001
