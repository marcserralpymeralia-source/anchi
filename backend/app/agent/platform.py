...
from app.agent.prompt_runtime import record_external_prompt_execution
...
class UnifiedOrderPipelineService:
    ...
    def process_inbound_message(
        ...
        try:
            ...
            if email_fast_path:
                ...
                structured, fast_state, fast_message = self._extract_structured_fast_path(
                    llm_settings,
                    normalized,
                    inbound_message,
                    db=db,
    ...
    def _extract(self, db: Session, settings: LLMSettings, company_id: int, text: str, inbound_message: InboundMessage | None = None) -> dict[str, Any]:
        structured, structured_error = self._extract_structured(settings, text, inbound_message, db=db, company_id=company_id)
        if structured:
        ...
        *,
    ...
    ) -> tuple[OrderExtractionResult | None, str | None]:
        ...
        try:
            result = extract_order(
                ...
                input_reference=f"inbound_message:{inbound_message.id}" if inbound_message and inbound_message.id else None,
                trace_callback=self._structured_trace_callback(db, company_id, settings),
            )
        except Exception as exc:
    ...
    @staticmethod
    def _structured_trace_callback(db: Session | None, company_id: int | None, settings: LLMSettings):
        if db is None or company_id is None:
            return None

        def persist_trace(trace: dict[str, Any]) -> None:
            record_external_prompt_execution(
                db,
                company_id,
                "extraction",
                settings,
                prompt_name="Extracción estructurada de pedidos",
                model=str(trace.get("model") or "unknown"),
                system_prompt=str(trace.get("system_prompt") or ""),
                user_input=str(trace.get("user_input") or ""),
                assistant_output=trace.get("assistant_output"),
                output_status=str(trace.get("output_status") or "error"),
                validation_errors=trace.get("validation_errors") if isinstance(trace.get("validation_errors"), list) else None,
                input_reference=trace.get("input_reference"),
                duration_ms=int(trace.get("duration_ms") or 0),
                provider_metadata=trace.get("provider_metadata") if isinstance(trace.get("provider_metadata"), dict) else None,
                decision_summary=trace.get("decision_summary") if isinstance(trace.get("decision_summary"), str) else None,
            )
