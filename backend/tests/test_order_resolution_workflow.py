...
class OrderResolutionWorkflowTests(unittest.TestCase):
    ...
    def test_manual_whatsapp_preview_process_and_confirm(self):
        ...
        self.assertEqual(confirm_response.headers["location"], "/channels?focus=inbound-1")
    ...
    def test_unified_resolution_route_handles_email_and_inbound_sources(self):
        ...
        self.assertEqual(inbound_process_response.headers["location"], f"/channels?focus=inbound-{inbound_message.id}")
        ...
        self.assertEqual(inbound_response.headers["location"], f"/channels?focus=inbound-{inbound_message.id}")
        self.assertEqual(db.scalar(select(func.count()).select_from(BackgroundJob)) or 0, 2)
